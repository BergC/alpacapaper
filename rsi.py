...
ticker_count = db[f'{today}: Below Zero'].estimated_document_count()
...
for ticker in below_zero_crossovers:
    ...
    def rsi_prior(lookback):
        ...
        if day_of_week <= 4:
            ...
            # print(float(r_json['Technical Analysis: RSI'][lookback_date_str]['RSI']))
    ...
    if r_json['Technical Analysis: RSI'] and rsi(today) > 30 and rsi(today) > rsi_prior(1) and rsi_prior(1) > rsi_prior(2):
         db[f'{today}: Below Zero'].update_one(
             {
