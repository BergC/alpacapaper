...
for ticker in today_tickers_list:
    try:
        ...
        if num_calls != 0 and num_calls % 5 == 0:
            ...
            print(f'Sleeping, {num_calls} Stochastic Oscillator\'s completed so far. We have {num_remaining} tickers left.')
        ...
        stoch_k = float(r_json['Technical Analysis: STOCH'][today]['SlowK'])
        stoch_d = float(r_json['Technical Analysis: STOCH'][today]['SlowD'])

        # Push buy sign cross overs below the zero line to their own collection.
        if stoch_d < 20 and stoch_k > stoch_d:
        ...
        err_ticker = ticker['ticker']
        print(f'{num_calls} calls completed and {err_ticker} threw an error.')
        num_calls += 1
        continue
